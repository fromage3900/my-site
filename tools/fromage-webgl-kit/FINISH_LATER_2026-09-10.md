# Fromage WebGL Kit — Finish Later Queue
...
Earn the first externally funded inference budget by converting existing or near-complete WebGL capability into one bounded paid result.
...
**Current state:** source seeded; custom handheld asset assigned as the highest-value original asset proof; runtime evidence still required.

Read:

- `DIRECT_LISTING_STRIKES_2026-09-10.md` — Strike 1;
- `CUSTOM_ASSET_STRIKE_PACK_2026-09-10.md` — Asset 1;
- `prototypes/product-motion-lab/`.

## Next actions

1. Finish the original handheld product blockout/hierarchy.
2. Export or integrate the original asset into Product Motion Lab.
3. Verify deterministic scrub `0 → 1 → 0`.
4. Verify three callout anchors remain correctly attached to moving subcomponents.
5. Record triangles, materials, draw calls, DPR, approximate FPS and actual asset bytes if a GLB exists.
6. Capture desktop and phone-width evidence.
7. Write/update `LISTING_3000_EVIDENCE.md` with only observed facts.
8. Recheck the listing is still active before spending proposal effort.
9. Draft a concise human-reviewed application only after evidence exists.

## Do not add

- React/R3F rewrite solely for optics;
- ecommerce;
- backend;
- client-product imitation;
- broad portfolio redesign;
- arbitrary extra product features.
...
- existing `wix/sdf-material-gallery.html`.
...
- duplicate fabric application;
- physical-BRDF accuracy claims without measurement;
- fake automated swatch-scanning claims;
- ecommerce/order backend;
- arbitrary user uploads for the first proof;
- claims of being a CLO specialist unless the shown work supports that wording.
...
- `prototypes/procedural-industrial-loop/README.md`;
...
- physics framework;
- camera spectacle that obscures the motion-law proof;
- broad Houdini pipeline unless a real buyer/delivery need justifies it.

---

# Shared evidence standard

Every claim used in an application must map to one of:
...
Never promote a source-intended feature into a runtime claim before it has been observed.

Use these status words consistently:

- `SOURCE-SEEDED` — code exists but is incomplete or only a starting point;
- `SOURCE IMPLEMENTED` — intended feature is present in source;
- `RUNTIME VERIFIED` — observed in a real execution environment;
- `CAPTURED` — evidence still/image/video exists;
- `PROPOSAL READY` — listing is current, must-haves are truthful, and relevant evidence exists;
...
A strike can become `PROPOSAL READY` only when:

1. the listing is still active/current;
2. relevant runtime evidence exists;
3. every must-have we claim can be demonstrated or honestly described as prior owner experience;
4. missing requirements are disclosed rather than implied;
5. no client-specific unpaid deliverable was produced;
6. the economics still justify the human time;
7. proposal submission remains a human decision.

---

# End-of-session handoff template

At the end of any later work session, append/update the relevant evidence file with:
...
The most important field is `NEXT SINGLE ACTION`. Do not leave future agents a broad research assignment when one concrete execution step is known.
